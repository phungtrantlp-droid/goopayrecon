...
import { TrendingDown, Upload, Download, Edit2, Trash2, Lock } from 'lucide-react';
...
export const ChiPage: React.FC = () => {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const res = await getTransactions({ ...filters, type: TransactionType.CHI });
      setData(res);
    } catch (e) {
  ...
  const handleUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file) return;
    try {
      const res = await uploadTransactions(file, TransactionType.CHI);
    ...
    <div className="flex flex-col gap-4 animate-fade-in h-[calc(100vh-100px)]">
      <div className="flex items-center justify-between shrink-0">
        <div className="flex items-center gap-3 text-white">
          ...
          <h1 className="text-2xl font-bold">Phát Sinh Chi</h1>
        </div>
        {canEdit() && (
          <div className="flex gap-3">
            <Button variant="outline" leftIcon={<Download className="w-4 h-4" />} onClick={async () => {
              const blob = await downloadTemplate(TransactionType.CHI);
              const url = window.URL.createObjectURL(blob);
              const a = document.createElement('a'); a.href = url; a.download = 'chi_template.xlsx'; a.click();
            }}>Template</Button>
      ...
      <div className="shrink-0 glass-card p-4 rounded-lg flex justify-between items-center text-sm font-medium">
        ...
        <div className="flex gap-8">
          ...
          <div className="text-gray-400">Tổng Số Tiền: <span className="text-red-400 ml-1 text-lg">{formatCurrency(totalAmount)}</span></div>
        </div>
      </div>

      <Modal isOpen={isUploadOpen} onClose={() => setIsUploadOpen(false)} title="Upload File Excel - CHI" size="md">
      ...
      <Modal isOpen={isEditOpen} onClose={() => setIsEditOpen(false)} title="Sửa Giao Dịch Chi" footer={<><Button variant="ghost" onClick={() => setIsEditOpen(false)}>Hủy</Button><Button onClick={handleSaveEdit}>Lưu</Button></>}>
        {editingRow && (
